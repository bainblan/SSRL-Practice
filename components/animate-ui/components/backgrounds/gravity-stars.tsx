...
type GravityStarsProps = {
  ...
  /** Track mouse at the window level so interaction still works through pointer-events: none. */
  trackWindowMouse?: boolean;
  /** Shift stars by the page scroll delta for a "traveling through space" feel. 0 disables. */
...
function GravityStarsBackground({
  ...
  // Window-level mouse tracking — works even when the container has pointer-events: none
  React.useEffect(() => {
  ...
  // Scroll-driven parallax — shift stars by scroll delta, wrapping within the canvas
  React.useEffect(() => {
    if (!scrollParallax) return;
    let lastScroll = typeof window !== 'undefined' ? window.scrollY : 0;
    const onScroll = () => {
      const cur = window.scrollY;
      const dy = (cur - lastScroll) * scrollParallax;
      lastScroll = cur;
      const h = canvasSize.height;
      if (h <= 0) return;
      for (const p of starsRef.current) {
        p.y -= dy;
        // wrap within canvas bounds (handles any magnitude of dy)
        p.y = ((p.y % h) + h) % h;
      }
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    return () => window.removeEventListener('scroll', onScroll);
  }, [scrollParallax, canvasSize.height]);
